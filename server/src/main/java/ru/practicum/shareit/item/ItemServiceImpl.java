...
public class ItemServiceImpl implements ItemService {
	...
	public ItemServiceImpl(ItemRepository itemStorage, UserRepository userStorage, BookingRepository bookingStorage,
						   CommentRepository commentStorage, ItemRequestRepository itemRequestStorage) {
	...
	@Override
	@Transactional
	public ItemDto createItem(RequestItemDto dto, long userId) {
		User user = userStorage.findById(userId).orElseThrow(() -> new NotFoundException("Пользователь не найден"));
		Item item = ItemMapper.toItem(dto);
		item.setOwner(user);
		if (dto.getRequestId() != null) {
			item.setRequest(itemRequestStorage.findById(dto.getRequestId()).get());
		}
	...
	@Override
	@Transactional
	public ItemDto updateItem(RequestItemDto dto, long userId, long itemId) {
